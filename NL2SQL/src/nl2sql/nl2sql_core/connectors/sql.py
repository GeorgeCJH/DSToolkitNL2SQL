...
import json
from ..utils.database import DatabaseEngineSpecificFields
import re
import sys

# CRITICAL: Windows async compatibility fix
if sys.platform == "win32":
    try:
        current_policy = asyncio.get_event_loop_policy()
        if not isinstance(current_policy, asyncio.WindowsSelectorEventLoopPolicy):
            asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())
            logging.info("SQL Connector: Set event loop policy to WindowsSelectorEventLoopPolicy")
    except Exception as e:
        logging.warning(f"SQL Connector: Could not set event loop policy: {e}")
...
class SqlConnector(ABC):
    ...
    async def get_column_values(
        ...
        # Commenting out for less verbose logging
    ...
    async def query_execution_with_limit(
        ...
        if validation_result and validation_errors is None:
            try:
                ...
                # Ensure result is always a list for Pydantic validation
                if result is None:
                    result = []
                elif not isinstance(result, list):
                    logging.warning(f"query_execution returned {type(result)}, converting to list")
                    result = []
                ...
                logging.error(f"Error in query_execution_with_limit: {e}")
                # Return error response with empty sql_rows list
                return json.dumps(
                    {
                        "type": "errored_query_execution_with_limit",
                        "sql_query": cleaned_query,
                        "sql_rows": [],  # Always provide empty list
                        "errors": [str(e)],
                    },
                    default=str,
                )
