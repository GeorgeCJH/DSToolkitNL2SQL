...
`./nl2sql_core/data_dictionary/data_dictionary_creator.py` contains a utility class that handles the automatic generation and selection of schemas from the source SQL database. It must be subclassed to the appropriate engine to handle engine specific queries and connection details.
...
- **Databricks:** `./text_2nl2sql_core_sql_core/data_dictionary/databricks_data_dictionary_creator.py`
- **Snowflake:** `./nl2sql_core/data_dictionary/snowflake_data_dictionary_creator.py`
- **TSQL:** `./nl2sql_core/data_dictionary/tsql_data_dictionary_creator.py`
- **Postgres:** `./nl2sql_core/data_dictionary/postgres_data_dictionary_creator.py`
...
1. Create your `.env` file based on the provided sample `nl2sql/.env.example`. Place this file in the same place in `nl2sql/.env`.

**Execute the following commands in the `nl2sql_core` directory:**

2. Package and install the `nl2sql_core` library. See [build](https://docs.astral.sh/uv/concepts/projects/build/) if you want to build as a wheel and install on an agent. Or you can run from within a `uv` environment and skip packaging.
