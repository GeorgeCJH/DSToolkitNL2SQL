...
def deploy_config(arguments: argparse.Namespace):
    ...
    elif arguments.index_type == "nl2sql_column_value_store":
        index_config = Text2SqlColumnValueStoreAISearch(
            suffix=suffix,
            rebuild=arguments.rebuild,
        )
    else:
        raise ValueError(f"Invalid Indexer Type: {arguments.index_type}. Valid types are: nl2sql_schema_store, nl2sql_query_cache, nl2sql_column_value_store")
